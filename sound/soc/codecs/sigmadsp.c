...
static int sigmadsp_rate_to_index(struct sigmadsp *sigmadsp, unsigned int rate)
{
	...
	for (i = 0; i < sigmadsp->num_rates; i++) {
		if (sigmadsp->rates[i] == rate)
			return i;
	}
	...
	const struct sigma_fw_chunk *chunk, unsigned int length)
{
	...
	if (sigmadsp->num_rates)
		return -EINVAL;

	sigmadsp->rates = kcalloc(num_rates, sizeof(*sigmadsp->rates), GFP_KERNEL);
	if (!sigmadsp->rates)
		return -ENOMEM;

	sigmadsp->num_rates = num_rates;

	for (i = 0; i < num_rates; i++)
		sigmadsp->rates[i] = le32_to_cpu(rate_chunk->samplerates[i]);
