...
{
	struct pata_pxa_data *pd = param;

	dma_unmap_sg(pd->dma_channel->device->dev,
		     pd->sg, pd->dma_len, pd->dma_dir);
...
static void pxa_qc_prep(struct ata_queued_cmd *qc)
{
	...
	/*
	 * I am not sure I can pass the qc->sg through private data here...
	 * Can this function be entered multiple times until the IRQ happens,
	 * which would thus corrupt this pd->sg here?
	 *
	 * Surely, we cannot pass 'qc' as the IRQ callback param, since the
	 * 'qc' is de-allocated right after this function finishes, which can
	 * happen before the IRQ happens.
	 */
	pd->sg = qc->sg;
