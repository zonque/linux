 ...
 *
 *   Note that the cgroup pointed to has to have the bpf controller
 *   enabled. Run "echo +bpf >cgroup.subtree_control" in all ancestors
 *   so "bpf" appears in that cgroup's "cgroup.controllers" file.
 *
...
static int prog_load(int map_fd, int verdict)
{
	struct bpf_insn prog[] = {
		...
		BPF_MOV64_IMM(BPF_REG_0, verdict), /* r0 = bpf_verdict */
		BPF_EXIT_INSN(),
	};
...
static int usage(const char *argv0)
{
	printf("Usage: %s <cg-path> egress|ingress [drop]\n\n"
	       "The cgroup has to be under control of the bpf controller\n"
	       "(\"echo +bpf' >cgroup.subtree_control\" in all ancestors)\n",
	       argv0);
	return EXIT_FAILURE;
}
